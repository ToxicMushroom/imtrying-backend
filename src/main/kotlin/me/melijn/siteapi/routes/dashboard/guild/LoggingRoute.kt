...
class LoggingRoute {
    ...
    class Set : AbstractRoute("/setsettings/logging", HttpMethod.Post) {
        override suspend fun execute(context: IRouteContext) {
            ...
            val url = "${context.getMelijnHost(guildId)}/setsettings/logging/$guildId"
    ...
    class Get : AbstractRoute("/getsettings/logging", HttpMethod.Post) {
        override suspend fun execute(context: IRouteContext) {
            val guildId = getGuildId(context) ?: return
            val userId = getUserInfo(context)?.idLong ?: return

            val url = "${context.getMelijnHost(guildId)}/getsettings/logging/$guildId"
